...
import (
	...
	quic "github.com/bassosimone/quic-go"
	mockquic "github.com/bassosimone/quic-go/internal/mocks/quic"
...
var _ = Describe("RoundTripper", func() {
	...
	Context("dialing hosts", func() {
		...
		BeforeEach(func() {
			...
			origDialAddr = dialAddr
			dialAddr = func(ctx context.Context, addr string, tlsConf *tls.Config, config *quic.Config) (quic.EarlySession, error) {
		...
		It("uses the custom dialer, if provided", func() {
			var dialed bool
			dialer := func(ctx context.Context, _, _ string, tlsCfgP *tls.Config, cfg *quic.Config) (quic.EarlySession, error) {
				dialed = true
				return nil, errors.New("handshake error")
			}
			rt.Dial = dialer
